...
def test_load_snapshots_parses_market_fields(tmp_path):
    """验收：all.csv 数值化 + 日期字符串保留 + date 回传。"""
    d = _scan_dir(tmp_path)
    snap = scan.load_snapshots(dir=str(d))
    m = snap["market"]["AKEUSDT"]
...
def test_load_snapshots_boards_dedup_preserve_order(tmp_path):
    """验收：movers 同币多行 board 去重保序（对齐用户表格三榜）。"""
    d = _scan_dir(tmp_path)
    boards = scan.load_snapshots(dir=str(d))["market"]["AKEUSDT"]["boards"]
    assert boards == ["gain_1h", "loss_24h", "gain_7d"]


def test_load_snapshots_microstructure_fields(tmp_path):
    """验收：microstructure.csv 数值化 + funding_trend 字符串保留。"""
    d = _scan_dir(tmp_path)
    mic = scan.load_snapshots(dir=str(d))["microstructure"]["AKEUSDT"]
...
def test_load_snapshots_missing_dir_and_files(tmp_path, monkeypatch):
    """验收：目录/文件缺失 → {}（绝不抛异常，决策链占位）。"""
...
def test_load_snapshots_corrupt_file_returns_empty(tmp_path):
    """验收：损坏文件（非法 UTF-8）→ {}，绝不抛异常。"""
    d = tmp_path / "scan"
    d.mkdir()
    (d / "2026-08-16_all.csv").write_bytes(b"\xff\xfe\x00broken")
    assert scan.load_snapshots(dir=str(d)) == {}
